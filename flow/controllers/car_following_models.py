...
class IDMController(BaseController):
    ...
    def get_accel_diff(self, env, pos, vel):
        ...
        # v = vel # use vel argument
        lead_id = env.k.vehicle.get_leader(self.veh_id)
        lead_pos = env.k.vehicle.get_position(lead_id)
        veh_length = env.k.vehicle.get_length(self.veh_id)

        h = lead_pos - pos - veh_length # use argument

        # print("vehicle length: ", veh_length)
        # h = env.k.vehicle.get_headway(self.veh_id)

        # in order to deal with ZeroDivisionError
        # cond = tf.cond(tf.greater(1e-3, h), lambda: True, lambda: False)
        ...
        return self.a * (1 - (vel / self.v0)**self.delta - (s_star / h)**2)
    ...
    def get_accel(self, env):
        ...
        # print("vehicle length: ", env.k.vehicle.get_length(self.veh_id))
