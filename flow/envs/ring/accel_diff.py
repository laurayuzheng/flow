...
from flow.envs.base_diff import DiffEnv
...
class AccelDiffEnv(DiffEnv):
    ...
    * sort_vehicles: specifies whether vehicles are to be sorted by position
    ...
    # state is a tensorflow tensor
    # @tf.function
    def compute_reward_diff(self, rl_actions, state, **kwargs):
        """See class definition."""
        # speeds = [state[id*2+1] for id in range(state.get_shape()[0] // 2)]
        # shape = state.get_shape()
        # print(shape[0])
        # state_reshape = tf.reshape(state, [int(shape[0])/2, 2])
        mean = torch.mean(state[1::2])
        # mean = tf.math.reduce_mean(state)
        # mean = state * 2
        # if self.env_params.evaluate:
        return mean
        # else:
