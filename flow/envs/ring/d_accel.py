...
class dAccelEnv(DiffEnv):
    ...
    * sort_vehicles: specifies whether vehicles are to be sorted by position
    ...
    def compute_reward(self, rl_actions, **kwargs):
        """See class definition."""
        # return torch.tensor([torch.mean(kwargs['next_state'][1::2])])
        if self.env_params.evaluate:
            return np.mean(kwargs['next_state'][1::2])
        else:
            return rewards.desired_velocity(self, fail=kwargs['fail'])

    def d_compute_reward(self, rl_actions, rl_indices, state, **kwargs):
        return rewards.d_desired_velocity(state, self.env_params.additional_params['target_velocity'], fail=kwargs['fail'])
