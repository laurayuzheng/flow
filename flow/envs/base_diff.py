...
class DiffEnv(gym.Env, metaclass=ABCMeta):
    ...
    * compute_reward
    ...
    def construct_IDM_jac(self,grad_dict):
        jac = np.zeros((2*len(self.sorted_ids), 2*len(self.sorted_ids)))
        controlled_ids = self.k.vehicle.get_controlled_ids()

        for i,veh_id in enumerate(self.sorted_ids):
            # if it is an RL vehicle, acceleration gradient 
            # has no relation, it is dictated by RL alg
            if veh_id not in controlled_ids or veh_id not in grad_dict:
                continue

            lead_id = self.k.vehicle.get_leader(veh_id)
            gradf,gradg = grad_dict[veh_id]

            jac[2*i, 2*i] = 0. # 0 + 1 for update
            jac[2*i, 2*i + 1] = 1. 

            if lead_id is None or lead_id == '': 
                jac[2*i + 1, 2*i] = 0
                jac[2*i + 1, 2*i + 1] = gradg #+ 1. # +1 for update
                continue # no need to worry about subdiag
            else: 
                sub1,sub2 = self.k.vehicle.get_acc_controller(veh_id) \
                                .get_subdiag_diff(self, 
                ...
                jac[2*i, 2*i - 2] = 0. 
                jac[2*i, 2*i - 1] = 0. 
                jac[2*i + 1, 2*i - 2] = sub1
                jac[2*i + 1, 2*i - 1] = sub2

        return jac

    def calc_jac(y,x, free=False): # autograd
        ans = []
        py = y.reshape([-1])
    ...
    def update_q(self, q_0, rl_actions=[]):
        
        q = torch.clone(q_0)
        action = 0
        rl_i = 0
        num_vehicles = int(q.size(dim=0)) / 2
        for i, veh_id in enumerate(self.sorted_ids):
            if i < num_vehicles:
                veh_pos = q.clone()[2*i]
                veh_vel = q.clone()[2*i+1]

                if veh_id in self.k.vehicle.get_controlled_ids():
                    controller = self.k.vehicle.get_acc_controller(veh_id)
                    action = controller.get_action_diff(self, veh_pos, veh_vel)
                elif veh_id in self.k.vehicle.get_rl_ids():
                    action = rl_actions[i] # get action for RL 
                    rl_i += 1

                q[2*i] = veh_pos + veh_vel*self.sim_step
                q[2*i+1] = veh_vel + action*self.sim_step
        
        return q
                

    def step(self, rl_actions, lane_change=False): # assumes Controller is IDM
        ...
        start_state = torch.tensor(np.array(self.get_state()), requires_grad=True)

        if rl_actions: 
            rl_actions_list = list(rl_actions)
            rl_actions_torch = torch.tensor(rl_actions, requires_grad=True) 
        else: 
            rl_actions_list = [] 
            rl_actions_torch = None 

        # print(start_state)

        # with tf.GradientTape() as tape: # autograd
        # tape.watch(start_state)
        # state_var = tf.identity(start_state).numpy() # make a copy
        # tape.watch(state_var)
        ...
        for _ in range(self.env_params.sims_per_step):
            ...
            if len(self.k.vehicle.get_controlled_ids()) > 0:
                accel = []
                actions = []
                first_veh = self.k.vehicle.get_controlled_ids()[1] #second veh
                # for i, veh_id in enumerate(self.k.vehicle.get_controlled_ids()):
                for i, veh_id in enumerate(self.sorted_ids):
                    if veh_id in self.k.vehicle.get_controlled_ids():
                        # print("i: ", i)
                        # print("veh id: ", veh_id)
                        # veh_pos = tf.Variable(self.k.vehicle.get_position(veh_id))
                        # veh_vel = tf.Variable(self.k.vehicle.get_speed(veh_id))

                        # veh_pos = torch.tensor(self.k.vehicle.get_position(veh_id), requires_grad=True)
                        # veh_vel = torch.tensor(self.k.vehicle.get_speed(veh_id), requires_grad=True)
                        # print("Pos from kernel: ", veh_pos)
                
                        veh_pos = torch.tensor(start_state.clone()[2*i], requires_grad=True)
                        veh_vel = torch.tensor(start_state.clone()[2*i+1], requires_grad=True)
                        # print("Pos from state: ", veh_pos)
                        # print() 
                        ...
                        # grad = tape.gradient(action, [veh_pos, veh_vel])
                        # grad_pos = veh_pos.grad 
                        # grad_vel = veh_vel.grad 
                        # gradient_dict[veh_id] = [grad_pos.numpy(), grad_vel.numpy()]

                        # manual diff
                        action = controller.get_action(self)
                        gradf, gradg = self.k.vehicle.get_acc_controller(
                                veh_id).get_d_action(self)
                        gradient_dict[veh_id] = [gradf, gradg]
                        

                        # if veh_id == first_veh:

                            # autograd
                            # action_grad1 = tape.jacobian(action, [veh_pos, veh_vel])
                            # print("Auto grad wrt pos: ", grad_pos)
                            # print("Auto grad wrt vel: ", grad_vel)

                            # sanity check ; manual gradient
                            # action_grad_x, action_grad_v = self.k.vehicle.get_acc_controller(
                            #     veh_id).get_d_action(self)
                            # # if veh_id == first_veh: # see grad of accel for one vehicle
                            # print("Manual grad of accel wrt pos: ", action_grad_x)
                            # print("Manual grad of accel wrt vel: ", action_grad_v)
                            # print()
                        accel.append(action)
                        actions.append(action) # add to all actions
                    elif veh_id in self.k.vehicle.get_rl_ids(): 
                        actions.append(rl_actions_list.pop(0)) # add to all actions

                # core.kernel.vehicle.traci.apply_acceleration
                self.k.vehicle.apply_acceleration(
                    self.k.vehicle.get_controlled_ids(), accel)

                # copied from traci.py in flow.core.kernel.vehicles.traci
                # accel_ind = 0
                # for i, vid in enumerate(self.sorted_ids):
                #     if accel[accel_ind] is not None and vid in self.k.vehicle.get_ids() and vid in self.k.vehicle.get_controlled_ids():
                #         state_var[2*i] = max([state_var[2*i] + state_var[2*i+1] * self.sim_step, 0])
                #         state_var[2*i+1] = max([state_var[2*i+1] + accel[accel_ind] * self.sim_step, 0])
                #         accel_ind += 1
        ...
        states = self.get_state() # assumes AccelEnv
        states_manual = self.update_q(start_state, actions)
        # states_manual.backward() 
        print("grad q_t to q_t-1: \n", start_state.grad)
        # states_calc = start_state + 
        # collect information of the state of the network based on the
        # environment class used
        self.state = np.asarray(states).T
        # start_state = tf.convert_to_tensor(states, dtype=tf.float32)
        ...
        # state_var = tf.convert_to_tensor(state_var, dtype=tf.float32)
        # print(start_state)
        # start_state.assign(state_var)
        # print(start_state)
        # states = tf.convert_to_tensor(states, dtype=tf.float32)
        states = torch.tensor(states, requires_grad=True)
        # print(gradient_dict)
        # d action wrt state
        jac = self.sim_step * torch.tensor(self.construct_IDM_jac(gradient_dict)) # manual 
        ones = torch.ones(jac.shape[0])
        ones = torch.diag(ones, 0)
        jac = jac + ones
        jac2 = jacobian(self.update_q, (start_state))
        ...
        if self.env_params.clip_actions:
            ...
            # reward = self.compute_reward_diff(rl_clipped, states, fail=crash)
            reward = self.compute_reward_diff(rl_clipped, states_manual, fail=crash)
        else:
            # reward = self.compute_reward_diff(rl_actions, states, fail=crash)
            reward = self.compute_reward_diff(rl_clipped, states_manual, fail=crash)
        ...
        # start_manual_grad = states_manual.grad
        start_state_grad = start_state.grad 
        # print("Start state grad: \n", start_state_grad)
        # print("Start manual grad: \n", start_manual_grad)
        # print(jac.shape)
        # print(start_manual_grad.numpy().shape)

        # grad(new states wrt old states) * grad(reward wrt new states)
        # total_grad = jac @ start_manual_grad.numpy() 
        
        # print("Total grad (old): \n", total_grad)
        # print("End to end grad: \n", start_state_grad)
        start_state.grad.zero_()
